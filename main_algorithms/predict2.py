from train_id32 import p1,categories
import os,process_text 
import pandas as pd 
import numpy as np
import math
from hyperparameters2 import per,number_of_vocab_words,approximate_logs_inside_id3,approximate_logs
folders = ['neg','pos']
...
f = open(file_path,"a")
f.write(str(math.ceil(per)) + "," + str(accuracy) + "," + str(precision) + "," + str(recall) + "," + str(number_of_vocab_words) + "\n")
f.close()
